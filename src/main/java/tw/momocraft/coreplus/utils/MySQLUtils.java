...
public class MySQLUtils implements MySQLInterface {
    ...
    private PreparedStatement getStatement(String pluginName, String databaseType, String sql) {
        try {
            switch (databaseType) {
                ...
                default:
                    UtilsHandler.getLang().sendErrorMsg(databaseType, "Failed to get the value from MySQL database: " + databaseType);
                    UtilsHandler.getLang().sendErrorMsg(databaseType, "Please check whether CorePlus is the latest version.");
                    return null;
            }
        } catch (SQLException ex) {
            UtilsHandler.getLang().sendErrorMsg(databaseType, "Failed to get the value of MySQL database: " + databaseType);
            UtilsHandler.getLang().sendErrorMsg(databaseType, "sql = " + sql);
            UtilsHandler.getLang().sendDebugTrace(true, pluginName, ex);
            return null;
        }
    }
    ...
    public void addColumns(String pluginName, String databaseType, String table, List<String> columns) {
        try {
            ...
            UtilsHandler.getLang().sendErrorMsg(databaseType, "Failed to add column of MySQL database: " + databaseType);
            UtilsHandler.getLang().sendDebugTrace(true, pluginName, ex);
        }
    }
    ...
    public void executeSQL(String pluginName, String databaseType, String sql) {
        try {
            ...
            UtilsHandler.getLang().sendErrorMsg(databaseType, "Failed to execute SQL statement of MySQL database: " + databaseType);
            UtilsHandler.getLang().sendErrorMsg(databaseType, "sql = " + sql);
            UtilsHandler.getLang().sendDebugTrace(true, pluginName, ex);
        }
    }
    ...
    public void setValue(String pluginName, String databaseType, String table, String column, String columnValue) {
        ...
        try {
            ...
            UtilsHandler.getLang().sendErrorMsg(databaseType, "Failed to set the value of MySQL database: " + databaseType);
            UtilsHandler.getLang().sendErrorMsg(databaseType, "sql = " + sql);
            UtilsHandler.getLang().sendDebugTrace(true, pluginName, ex);
        }
    }
